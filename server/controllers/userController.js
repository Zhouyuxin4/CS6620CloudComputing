...
exports.searchJourneys = async (req, res) => {
  try {
    ...
    const requiredJourneysOneIds = requiredJourneysOne.map((journey) =>
    ...
    const journeyDetails = await JourneyDetails.find({
      // search in all journey details text
      journeyId: { $in: allJourneyId },
      journalText: regex,
    });
    // find which journeys these journey details belong
    const requiredJourneysTwoIds = journeyDetails.map((detail) =>
      detail.journeyId.toString()
    );

    // get all qualified ids
    const qualifiedIds = new Set([
      ...requiredJourneysOneIds,
      ...requiredJourneysTwoIds,
    ]);
    const requiredJourneys = await Journeys.find({
      _id: { $in: Array.from(qualifiedIds) },
    }).populate("details");
